...
const GlobalDestinations: Flight[] = [
  ...
  },
  {
    ...
    price: 999,
    details: ['Futuristic and traditional.', 'Exceptional cuisine.', 'Rich culture.'],
  },
  {
    city: 'Sydney',
    country: 'Australia',
    ...
    price: 1099,
    details: ['Opera House and Harbour Bridge.', 'Beautiful beaches.', 'Vibrant nightlife.'],
  },
  {
    city: 'Rome',
    country: 'Italy',
  ...
  },
  {
    ...
    price: 799,
    details: ['K-Pop culture.', 'Historic palaces.', 'Delicious street food.'],
  },
  {
    city: 'Amsterdam',
    country: 'Netherlands',
...
const ForeignDestinationCard: React.FC<{ GlobalGates: Flight; onEnquiryClick: (GlobalGates: Flight) => void }> = ({ GlobalGates, onEnquiryClick }) => {
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden">
      <img src={GlobalGates.image} alt={GlobalGates.city} className="w-full h-48 object-cover" />
      <div className="p-6">
        <h3 className="text-xl font-semibold text-gray-800 mb-2">
          {GlobalGates.city}, <span className="text-gray-600">{GlobalGates.country}</span>
        </h3>
        <div className="flex items-center justify-between mb-3">
          <span className="text-lg font-bold text-orange-500">${GlobalGates.price}</span>
        </div>
        <div>
          <h4 className="text-md font-semibold text-gray-700 mb-1">Highlights:</h4>
          <ul className="list-disc list-inside text-gray-600 text-sm">
            {GlobalGates.details.map((detail, index) => (
              <li key={index}>{detail}</li>
            ))}
          </ul>
        </div>
        <div className="mt-4">
          <p className="text-gray-700 text-sm italic">Interested in visiting {GlobalGates.city}?</p>
          <button
            ...
            onClick={() => onEnquiryClick(GlobalGates)}
          >
            Send Enquiry
          </button>
        </div>
...
const GlobalGates: React.FC = () => {
  ...
  const filteredDestinations = GlobalDestinations.filter((GlobalGates) =>
    GlobalGates.city.toLowerCase().includes(searchTerm.toLowerCase()) ||
    GlobalGates.country.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-6">
          <div className="text-gray-600 text-sm mb-2">
            ...
            <span className='text-orange-500 font-semibold'>Global Destinations</span>
          </div>
